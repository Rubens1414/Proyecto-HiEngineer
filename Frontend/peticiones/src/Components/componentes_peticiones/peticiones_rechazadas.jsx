...
import Peticiones_individuales_rechazadas from './peticiones_ind_r';
import '../../css/Principal.css'
import '../../css/solicitudes_canceladas.css'
...
function Peticiones_rechazadas() {
    ...
    const lista_peticiones = datapeticiones.map((usuario) => {
      return (
        <div key={usuario.id}>
          <Peticiones_individuales_rechazadas peticiones={usuario} />
        </div>
      );
    });
    return (
      <div>
      <div>
      <nav className="navegacion_solicitudes_canceladas">
           ...
           <a className="flex items-center ">
               ...
               {isOpen && (
                <div className=" esconder dropdown-container">
                  <ul className="dropdown-menu_solicitudes_canceladas">
             ...
             <div >
              <button className='m-8'onClick={() => setUserOpen(!userOpen)} ><FaUserCircle className='icon_desplazar_solicitudes_canceladas'/></button>
             
             {userOpen && (
                <div className="dropdown-user">
                  <div className="dropdown-menu-user_solicitudes_canceladas">
     ...
     <div>
         <div className='border-b-4 border-red-500' style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <h1 className='gugi text-stone-950 pt-2 pb-2 text-3xl'>Solicitudes rechazadas</h1>
         </div>
     ...
     {datapeticiones.length === 0 && (
           ...
           <h3 className=' font-thin mt-2'>No hay solicitudes rechazadas</h3>
            </div>
     )}
     </div>
...
export default Peticiones_rechazadas