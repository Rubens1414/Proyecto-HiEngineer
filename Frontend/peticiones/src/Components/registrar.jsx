...
}else if(registrado && tipo_user === 'no registrado'){
  Swal.fire({
    icon: 'success',
    title: 'Registro exitoso',
    color: 'white',
...
function agregarUsuario(){
  ...
  if (password.length < 8) {
    ...
    if (confirmacion === true && isAdmin === true) {
      campos.push('ninguno')
    }
    ...
    <h1 className="contenedor_registro_h1">
      ...
      <div className='flex flex-row justify-between'>
        ...
        {isAdmin === true && (
         ...
         {camposIncompletos.includes('ninguno') && isAdmin && (<p className="ml-4 text-sm text-red-500 dark:text-red-500" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}><span className="font-medium">Seleccione una opcion.</span>  </p>)}
